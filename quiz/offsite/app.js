    ...
    fetch('sounds.json').then(r => r.text()).then(data => {
        ...
        for (let j = 0; j < jsonFile.length; j++) {
            ...
            for (let i = 0; i < jsonFile[j]['contents'].length; i++) {
                ...
                newQuestion.innerHTML = `<div class="cont"><h3>${idCounter+1}</h3><input type="text" class="answer-box" id="input-${idCounter}" onchange="checkAnswer(${idCounter})" oninput="style.color = 'white'"></div><audio preload="none" id="audio-${idCounter}"><source src="${cdnURL}/${jsonFile[j]['contents'][i]['src']}"></audio>`
